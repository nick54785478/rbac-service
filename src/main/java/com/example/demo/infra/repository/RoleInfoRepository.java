...
public interface RoleInfoRepository extends JpaRepository<RoleInfo, Long> {
	...
	List<RoleInfo> findByServiceNot(String service);
	...
	default List<RoleInfo> findAllWithSpecification(String serviceName, String type, String name, String activeFlag) {
		Specification<RoleInfo> specification = ((root, query, cb) -> {
			List<Predicate> predicates = new ArrayList<>();

			if (StringUtils.isNotBlank(serviceName)) {
				predicates.add(cb.equal(root.get("service"), serviceName));
			}
